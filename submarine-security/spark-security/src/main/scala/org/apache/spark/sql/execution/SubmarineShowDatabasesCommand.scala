...
case class SubmarineShowDatabasesCommand(child: ShowDatabasesCommandCompatible) extends RunnableCommand {
  override val output = child.output

  override def run(sparkSession: SparkSession): Seq[Row] = {
    val catalog = sparkSession.sessionState.catalog
    val databases =
      CompatibleFunc.getPattern(child).map(catalog.listDatabases).getOrElse(catalog.listDatabases()).map { d => Row(d) }
