...
## Manage experiment code

Experiment code (such as training/batch-prediction) should be stored separately from library dependencies, data, etc. Ideally, experiment code should be versioned, and stored in version control. 
...
### Implementation of localization of experiment code 
...
For example, there's a git repo need to be synced up for an experiment (example above):
...
The above K8s spec create a code-dir and mount it to `/code` to launched containers. The initContiner `git-localize` uses `https://github.com/kubernetes/git-sync` to do the sync up. (If other storages are used, we can use similar initContainer approach to download contents).

## Manage storages for experiment

There're different types of storage, such as logs, metrics, dependencies (environments). For more details. Please refer to [storage-implementations](./storage-implementations,md) for more details.
