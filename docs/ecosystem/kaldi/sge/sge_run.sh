...
then
    sudo su - -c "sleep 30s && export DEBIAN_FRONTEND=noninteractive && apt-get install -y gridengine-master gridengine-exec gridengine-client;apt-get autoremove -y && apt-get clean && \
...
then
    ...
    sudo su - -c "export DEBIAN_FRONTEND=noninteractive &&  apt-get install -y gridengine-client gridengine-exec; apt-get autoremove -y && apt-get clean"
