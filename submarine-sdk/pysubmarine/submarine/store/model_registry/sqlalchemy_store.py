...
class SqlAlchemyStore(AbstractStore):
    ...
    def list_model_version(self, name: str, filter_tags: List[str] = None) -> List[ModelVersion]:
        ...
        with self.ManagedSessionMaker() as session:
            ...
            return [model_version.to_submarine_entity() for model_version in model_versions]
    ...
    def get_model_version_uri(self, name: str, version: int) -> str:
        ...
        with self.ManagedSessionMaker() as session:
            sql_model_version = self._get_model_version(session, name, version)
            return sql_model_version.to_submarine_entity().source

    @classmethod
    def _get_model_tag(cls, session: Session, name: str, version: int, tag: str) -> SqlModelTag:
        tags = (
            session.query(SqlModelTag)
            .filter(
                SqlModelTag.name == name, SqlModelTag.version == version, SqlModelTag.tag == tag
            )
            .all()
        )
        if len(tags) == 0:
    ...
    def add_model_tag(self, name: str, version: int, tag: str) -> None:
        ...
        with self.ManagedSessionMaker() as session:
            ...
            self._get_model_version(session, name, version)
            session.merge(SqlModelTag(name=name, version=version, tag=tag))
    ...
    def delete_model_tag(self, name: str, version: int, tag: str) -> None:
        ...
        with self.ManagedSessionMaker() as session:
            # check if model version exists
            self._get_model_version(session, name, version)
            existing_tag = self._get_model_tag(session, name, version, tag)
            session.delete(existing_tag)
